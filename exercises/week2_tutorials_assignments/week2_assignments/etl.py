...
def etl(path: Path, 
    ...
    context = gx.data_context.DataContext(gx_context_root_dir)
    batch_request = batch_creator(df, context, gx_datasource_name)
    checkpoint_result = create_checkpoint(context, batch_request, gx_checkpoint_name, gx_expectation_suite_name, gx_run_name)
